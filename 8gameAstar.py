...
def finish(dictAlreadySeen, numImp, numRem, numClosedSet, runTime):
    path = []
    counter = 1
...
if inversionCt(liststring)%2 != distanceRows(templiststring)%2:#(4 - (root.index('_')+1 // 4)) % 2:
    ...
    while notdone and openSet.qsize()>0:
        current = openSet.get(0)[2]
        if current == SOLVEDSTATE:
            notdone = False
            finish(alreadySeen, numTimesImproved, numTimesRemoved, len(closedSet), time.time()-starttime)
            continue
        closedSet.add(current)
        for n in neighbors(current):
            newpathlen = (manhattan(n), pathlens[current] + 1, n)
            if n in closedSet:
                continue
            if not is_in_queue(newpathlen, openSet):
                openSet.put(newpathlen)

            newpathlencount = pathlens[current] + 1
            if n in pathlens:
                if newpathlencount >= pathlens[n]:
                    continue
                numTimesImproved += 1
                numTimesRemoved += 1
...
#     else:
#         notin = 0
#         while keeplooping and parseMe.qsize() > 0:
